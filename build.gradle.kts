...
tasks.register("jarAndroid") {
    ...
    doLast {
        ...
        val command = mutableListOf<String>().apply {
            ...
            addAll(listOf("--min-api", "26", "--output", "${project.name}-android.jar", "${project.name}-desktop.jar"))
        }
...
tasks.register<Jar>("deploy") {
    ...
    from(provider {
        listOf(
            zipTree(layout.buildDirectory.file("libs/${project.name}-desktop.jar")),
            zipTree(layout.buildDirectory.file("libs/${project.name}-android.jar"))
        )
    })

    doLast {
        delete(layout.buildDirectory.file("libs/${project.name}-desktop.jar"))
