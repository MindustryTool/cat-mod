...
dependencies {
    ...
    implementation(project(":mdtdemo"))
...
tasks.register("jarAndroid") {
    ...
    doLast {
        ...
        val command = mutableListOf<String>().apply {
            ...
            addAll(listOf("--min-api", "26", "--output", "${rootProject.name}-android.jar", "${rootProject.name}-desktop.jar"))
        }
...
tasks.register<Jar>("deploy") {
    ...
    from(provider {
        listOf(
            zipTree(layout.buildDirectory.file("libs/${rootProject.name}-desktop.jar")),
            zipTree(layout.buildDirectory.file("libs/${rootProject.name}-android.jar"))
        )
    })

    doLast {
        delete(layout.buildDirectory.file("libs/${rootProject.name}-desktop.jar"))
        delete(layout.buildDirectory.file("libs/${rootProject.name}-android.jar"))
    }
}
...
tasks.register<JavaExec>("runGame") {
    ...
    workingDir = file("app-test")
    classpath = files("app-test/Mindustry.jar")
